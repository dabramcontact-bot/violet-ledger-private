...
function renderRequestRows(rows) {
  ...
  return `<div class="vl3-table-wrap"><table class="vl3-table"><thead><tr><th>Запрос</th><th>Товар / артикул</th><th>Поставщик</th><th>Дата</th><th>Этап</th><th></th></tr></thead><tbody>${rows.map(row => `<tr>
    <td><button class="vl3-link" data-action="edit-request" data-id="${row.id}">${escapeHtml(row.request_number)}</button></td>
    ...
    <div class="vl3-mobile-list">${rows.map(row => `<article class="vl3-mobile-card"><div class="vl3-mobile-head"><button class="vl3-link" data-action="edit-request" data-id="${row.id}">${escapeHtml(row.request_number)}</button>${stagePill(row)}</div><h3>${escapeHtml(row.product_name)}</h3><p>${escapeHtml(row.category)}${row.article_numbers ? ` · ${escapeHtml(row.article_numbers)}` : ''}</p>${renderRequestJourney(row)}<div class="vl3-mobile-meta"><span>${svg('factory', 14)}<small>Поставщик</small><b>${escapeHtml(row.agent_name)}</b></span><span>${svg('calendar', 14)}<small>Отправлен</small><b>${formatDate(row.request_sent_at)}</b></span></div>${canEdit() ? `<div class="vl3-mobile-actions"><button data-action="edit-request" data-id="${row.id}">${svg('edit', 15)} Редактировать</button><button class="danger" data-action="delete-request" data-id="${row.id}">${svg('trash', 15)}</button></div>` : ''}</article>`).join('')}</div>`
}
